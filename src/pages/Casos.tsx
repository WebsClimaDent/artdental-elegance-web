...
const Casos = () => {
  ...
  const renderStars = (rating: number) => {
    return Array.from({
      length: 5
    }, (_, index) => <Star key={index} className={`w-4 h-4 transition-all duration-200 ${index < rating ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`} />);
  };
  return <>
      <Header />
        ...
        <section className="py-16 md:py-20">
          <div className="container-custom">
            <div className="space-y-20">
              {cases.map((caseItem, index) => <div key={caseItem.id} className="reveal">
                  ...
                  {caseItem.id === 2 ? <div className="mb-8">
                      <ImageComparison beforeImage={caseItem.before} afterImage={caseItem.after} title={caseItem.title} />
                      ...
                      <div className="relative rounded-lg overflow-hidden">
                        ...
                        <img src={caseItem.before} alt={`${caseItem.title} - Antes`} className="w-full h-72 object-cover min-w-full " />
                      </div>
                      <div className="relative rounded-lg overflow-hidden">
                        <span className="absolute top-4 right-4 bg-white text-black text-sm px-3 py-1 rounded-md z-10">Después</span>
                        <img src={caseItem.after} alt={`${caseItem.title} - Después`} className="w-full h-72 object-cover object-center" />
                      </div>
                    </div>}
                  ...
                  {index < cases.length - 1 && <hr className="border-gray-700 mt-16" />}
                </div>)}
            </div>
          </div>
        </section>
        ...
        <section className="py-16 md:py-20 bg-gradient-to-b from-dental-dark to-black relative overflow-hidden">
          <div className="container-custom">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
              {reviews.map((review, index) => <div key={review.id} className={`group bg-white bg-opacity-10 backdrop-blur-sm border border-white border-opacity-20 rounded-lg p-6 shadow-lg transition-all duration-1000 hover:bg-opacity-15 hover:border-opacity-30 ${visibleReviews.includes(index) ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'} ${index === 2 ? 'md:col-span-2 lg:col-span-1' : ''}`} style={{
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex space-x-0.5">
                          {Array.from({
                        length: 5
                      }, (_, starIndex) => <Star key={starIndex} className={`w-4 h-4 transition-all duration-200 group-hover:animate-bounce ${starIndex < review.rating ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`} style={{
            ...
            <div className="text-center mt-16">
              ...
              <a href="https://maps.app.goo.gl/mhF5TiJ6WTngoVJa7" target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-white hover:text-gray-300 transition-colors">
