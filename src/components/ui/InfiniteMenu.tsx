...
void main() {
    ...
    // Special handling for carillas image (index 1) - show in original size without scaling
    if (itemIndex == 1) {
        // Reduce the image size significantly and center it
        float scale = 0.4; // Much smaller scale
        st = (st - 0.5) * scale + 0.5;
...
export default function InfiniteMenu({ items = [] }: InfiniteMenuProps) {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-gradient-to-br from-dental-dark via-gray-900 to-black">
      {/* Canvas - Special positioning for Coronas service to move background image up */}
      <canvas
        ref={canvasRef}
        className={`
          cursor-grab w-full h-full relative outline-none active:cursor-grabbing
      ...
      {activeItem && (
        <>
          {/* Content Container - Back to original position */}
          <div
            className={`
              absolute
          ...
          {/* Enhanced action button - Back to original position */}
          <div
